    ...
    <v-card-text>
      ...
      >
        <template v-slot:item.order_id="{value, item}">
          <router-link :to="`/cs/user_inquiries/${item.inquiry_idx}`">
            {{value}}
          </router-link>
        </template>
      </v-data-table>
...
<script>
    export default {
        ...
        async asyncData({store}) {
            try {
                const response = await store.dispatch('cs/fetchUserInquiries');
        ...
        watch: {
            options: {
                async handler(newVal) {
                    const response = await this.$store.dispatch('cs/fetchUserInquiries', {
                        page: newVal.page,
