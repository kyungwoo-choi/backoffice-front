              ...
              <v-row>
                <v-col cols="12" md="6">
                  ...
                  <dropzone id="product-main-image"
                            :options="{url: 'http://localhost:3000/', addRemoveLinks:true, maxFiles: 1}"
                            ref="main_image"
                ...
                <v-col cols="12" md="12">
                  <p class="body-1">상품 상세 이미지 업로드</p>
                ...
                <v-col cols="12" md="12">
                  <p class="body-1">상품 상세 이미지 목록</p>
                  <v-row>
                    <v-col cols="12" md="2" v-for="(imageInfo, index) of detailImages" :key="index">
                      <v-img :src="imageInfo.path">
                        <v-btn text @click="removeDetailImage(imageInfo)">
                          <v-icon>
                            mdi-close-circle
                          </v-icon>
                        </v-btn>
              ...
              <v-row>
                <v-col cols="12" md="12">
                  ...
                  >
                    ...
                    <template v-slot:item.add_price="{item, index}">
                      <v-text-field
                        placeholder="입력"
                        outlined
                        dense
    ...
    export default {
        ...
        async asyncData({store, params}) {
            ...
            try {
                ...
                response = await store.dispatch('product/getDetail', params.product_id);
                ...
                for (const brandItem of result.brandItems) {
                    if (brandItem.brand_id === product.brand_id) {
                        result.selectedBrand = brandItem;
                        break;
                    }
                }
                ...
                result.optionKeys = product.option_keys;
                result.detailImages = product.detail_images;

                result.selectedCategory = product.category_info;
                for (const category of product.category_info) {
                    if (category.children.length) {
                        result.categories.push(category.children)
                    }
                }
        ...
        methods: {
            ...
            async removeDetailImage(imageInfo) {
                if (!confirm('정말 이미지를 삭제하시겠습니까?')) {
                    return;
                }

                await this.$store.dispatch('product/deleteDetailImage', imageInfo)
            },
            changeCategory(index) {
                ...
                // 상위 카테고리 변경했을때
                if (this.selectedCategory[index] && index !== this.categories.length - 1) {
                    ...
                    if (this.selectedCategory[index].children.length) {
                        this.categories.push(this.selectedCategory[index].children);
                    }
                    // }, 10);
            ...
            async createProduct() {
                try {
                    ...
                    const response = await this.$store.dispatch('product/updateProduct', data);
